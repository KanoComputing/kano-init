...
class Drop(object):
    ...
    def _phase_one(self):
        if self._cycle % self._cycles_per_char == 0:
            c = ("0" if randint(0, 1) else "1")
            draw_fn(self._posy + self._charpos, self._posx, c, self._colour1)
            self._charpos += 1

            if self._charpos < self._length - 1:
                c = ("0" if randint(0, 1) else "1")
                draw_fn(self._posy + self._charpos, self._posx, c, 4)
    ...
    def _phase_two(self):
        if self._cycle % self._cycles_per_char == 0:
            c = ("0" if randint(0, 1) else "1")
            draw_fn(self._posy + self._charpos, self._posx, c, self._colour2)
            self._charpos += 1
...
def matrix_binary(duration=10, show_face=False):
    status = 1
...
if __name__ == "__main__":
    ...
    rv = matrix_binary(duration, show_face)
    sys.exit(rv)
